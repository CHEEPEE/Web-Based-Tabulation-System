...
function renderDashboard() {
  ReactDOM.render(
    <div>
      <SideNav ficon="grid" cname="sideNav-menus-font nav-link custom-active" lname={dashboard}/>
        ...
        <h5 className="mb-0">
          <button className="btn btn-link" data-toggle="collapse" data-target={"#collapseOne"+this.props.id} aria-expanded="true" aria-controls="collapseOne">
            <div className = "row">
              ...
              <div className = "col">
                <button type="button" onClick = {this.viewDetails.bind(this)} class="btn btn-outline-info m-1">View Details</button>
              </div>
            </div>
...
function getContestList(){
  ...
  var contestObjects = [];
  firebase.database().ref("events").once("value",function (dataSnapshot){
    ...
    var listItem = contestObjects.map((objects)=>
      <ContestList key = {objects.key} eventname = {objects.eventname} id = {objects.key}/>
    );
    ReactDOM.render(
      <div>
        {listItem}
      </div>,contestListContainer
    );
  });
}
